  ...
  return (
    <div style={{ background: 'var(--bg-page)', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{
        ...
        <button
          onClick={onBack}
          style={{ fontSize: 13, color: 'var(--text-muted)', background: 'transparent', border: 'none', cursor: 'pointer', padding: '4px 8px' }}
        ...
        {TABS.map(tab => (
          <button
            ...
            style={{
              ...
              color: activeTab === tab.id ? 'var(--text-primary)' : 'var(--text-very-muted)',
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {activeTab === 'files' && (
          sessions.length > 0
            ...
            : <div style={{ padding: 16, color: 'var(--text-very-muted)', fontSize: 13 }}>No sessions available for file browsing.</div>
        )}
      </div>
    </div>
  )
